...
pairwise_comparisons <-full_data_summary %>%
  pairwise_t_test(
    mean ~ trial_type_new, paired = TRUE,
    p.adjust.method = "holm")

effect_sizes <- full_data_summary %>%
  cohens_d(mean ~ trial_type_new, paired = TRUE, var.equal = FALSE)

# Step 3: Join the effect sizes with the t-test results
...
#   data.frame(participant = summary_stats_participant_LTM2$participant,
#              accuracy_ltm = accuracy_exp1,
#              experiment = "Exp1"),
#   data.frame(participant = summary_stats_participant_LTM_V32$participant,
#              accuracy_ltm = accuracy_exp2,
#              experiment = "Exp2")
# )
# 
# # Compute Cohen's d for independent samples
# effect_size <- combined_data %>%
#   cohens_d(accuracy_ltm ~ experiment, var.equal = FALSE)
# 
# # Print both the t-test and effect size
# t_test_result
# effect_size
...
differences <- accuracy_ltm_yes - accuracy_ltm_no
cohens_d_paired <- mean(differences) / sd(differences)

cat("Cohen's d (paired):", round(cohens_d_paired, 3), "\n")
...
pairwise_comparisons_ltm_V3 <-full_data_summary_ltm_V3 %>%
  pairwise_t_test(
    mean ~ trial_type, paired = TRUE,
    p.adjust.method = "holm")

effect_sizes_ltm_V3 <- full_data_summary_ltm_V3 %>%
  cohens_d(
    mean ~ trial_type, 
    paired = TRUE,
    var.equal = FALSE
...
# Step 3: Join the effect sizes to the t-test results
pairwise_comparisons_ltm_V3 <- pairwise_comparisons_ltm_V3 %>%
  left_join(effect_sizes_ltm_V3, by = c(".y.", "group1", "group2"))

# View the final result with effect sizes
pairwise_comparisons_ltm_V3
...
### In response to a reviwer comment, we are including a seperate ANOVA on the log transformed data to see how rubust the interaction effect between cueing and testing is


# Transform error to accuracy on a 0–1 scale
data_LTM_pruned_V3 <- data_LTM_pruned_V3 %>%
  mutate(
    accuracy_ltm_acc = (180 - accuracy_ltm) / 180  # Convert error to accuracy: 0 = worst, 1 = best
  )

# Adjust to avoid logit issues at 0 or 1
epsilon <- 0.001
data_LTM_pruned_V3 <- data_LTM_pruned_V3 %>%
  mutate(
    accuracy_ltm_acc_adj = pmin(pmax(accuracy_ltm_acc, epsilon), 1 - epsilon)
  )

# Logit transformation
data_LTM_pruned_V3 <- data_LTM_pruned_V3 %>%
  mutate(
    accuracy_ltm_logit = log(accuracy_ltm_acc_adj / (1 - accuracy_ltm_acc_adj))
  )

# Compute participant-level means by condition
participant_means_logit <- data_LTM_pruned_V3 %>%
  group_by(attention, testing, participant) %>%
  summarise(participant_mean = mean(accuracy_ltm_logit), .groups = "drop")

# Run repeated-measures ANOVA
res.aov_logit <- anova_test(
  data = participant_means_logit,
  dv = participant_mean,
  wid = participant,
  within = c(attention, testing)
)

# Output results
print("The ANOVA on the logit-transformed accuracy data yields an even stronger interaction effect.")
get_anova_table(res.aov_logit)
