...
pairwise_results <- WM_data_summary %>%
  pairwise_t_test(
    mean ~ trial_type_new, paired = TRUE,
    p.adjust.method = "holm",detailed = TRUE)

effect_sizes <- WM_data_summary %>%
  rstatix::cohens_d(mean ~ trial_type_new, paired = TRUE)
...
# Fit repeated-measures ANOVA with afex
fit_image_presWM <- aov_ez(
  id     = "participant",
  dv     = "mean",
  within = c("image_presentation", "trial_type_new"),
  data   = summary_image_pres
)

# Get partial eta² with 95% CI
eta_image_presWM <- eta_squared(
  fit_image_presWM,
  partial = TRUE,
  ci = 0.95
)
...
# Calculate Cohen's d
mean_diff <- mean(participant_means_summary$mean_accuracy)
sd_diff <- sd(participant_means_summary$mean_accuracy)
cohens_d <- (mean_diff - 0.5) / sd_diff
# Print Cohen's d
...
print("And here are all the possible tests with effect sizes for a better overview (not included in the manusscript")
...
effect_sizes_LTM_load <- data_summary_LTM_load %>%
  rstatix::cohens_d(mean ~ sample_trials, paired = TRUE)


pairwise_comparisons_LTM_load <- pairwise_comparisons_LTM_load %>%
  left_join(effect_sizes_LTM_load, by = c(".y.", "group1", "group2"))
...
# Compute partial eta² with 95% CI
eta_image_presWM <- eta_squared(
  fit_image_presWM,
  partial = TRUE,
  ci = 0.95
...
LTM_t_test2 <- summary_image_pres %>%
  group_by(trial_type_new) %>%
  pairwise_t_test(
    mean ~ image_presentation, paired = TRUE,
    p.adjust.method = "holm",detailed = TRUE
  )

# Add effect sizes per trial type
effect_sizes_LTM <- summary_image_pres %>%
  group_by(trial_type_new) %>%
  rstatix::cohens_d(mean ~ image_presentation, paired = TRUE)

# Join results
LTM_t_test2 <- LTM_t_test2 %>%
  left_join(effect_sizes_LTM, by = c("trial_type_new", ".y.", "group1", "group2"))
...
t_test_result <- t.test(mean_performance_LTM$accuracy_ltm,
                        mean_performance_LTM_V2$accuracy_ltm,
                        paired = FALSE)

# Calculate pooled standard deviation
sd1 <- sd(mean_performance_LTM$accuracy_ltm)
sd2 <- sd(mean_performance_LTM_V2$accuracy_ltm)
n1 <- length(mean_performance_LTM$accuracy_ltm)
n2 <- length(mean_performance_LTM_V2$accuracy_ltm)

pooled_sd <- sqrt(((n1 - 1)*sd1^2 + (n2 - 1)*sd2^2) / (n1 + n2 - 2))

# Compute Cohen's d
mean_diff <- mean(mean_performance_LTM$accuracy_ltm) - mean(mean_performance_LTM_V2$accuracy_ltm)
...
# Means
mean1 <- mean(mean_performance_LTM$accuracy_ltm)
mean2 <- mean(mean_performance_LTM_V2$accuracy_ltm)
