...
effect_sizes <- WM_data_summary %>%
  cohens_d(mean ~ trial_type_new, paired = TRUE)
...
effect_sizes_LTM_load <- data_summary_LTM_load %>%
  cohens_d(mean ~ sample_trials, paired = TRUE)

# Step 3: Join the effect sizes into the t-test results
...
effect_sizes_LTM <- summary_image_pres %>%
  group_by(trial_type_new) %>%
  cohens_d(mean ~ image_presentation, paired = TRUE)
